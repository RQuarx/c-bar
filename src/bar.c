...
create_bar(struct bar_widgets *widgets)
{
    ...
    gtk_css_provider_load_from_data(css_provider, css, 1002, nullptr);
...
create_window(GtkWindow **window)
{
    *window = GTK_WINDOW(gtk_window_new(GTK_WINDOW_TOPLEVEL));
    ...
    gtk_widget_set_size_request(GTK_WIDGET(*window), 1920, 21);
    gtk_window_resize(*window, 1920, 21);
}
